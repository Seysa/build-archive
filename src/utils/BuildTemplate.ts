...
export default class BuildTemplate {
    ...
    if (match !== null && match.length === 2) {
      ...
      if (bytes.length > 0 && bytes[0] !== 0x0d) {
        ...
        for (let s = 0; s < 3; s++) {
          ...
          for (let t = 0; t < 3; t++) {
            // 2 bit trait values
            this.specializations[s].traits[t] =
              (bytes[offset + 3] >> (t * 2)) & 0x03;
          }
        }
        ...
        for (let u = 0; u < 3; u++) {
          ...
          this.skills.terrestrial.utilities[u] =
            bytes[offset] + (bytes[offset + 1] << 8);
          this.skills.aquatic.utilities[u] =
  ...
  toString() {
    ...
    for (let u = 0; u < 3; u++) {
      retval.push(
        this.skills.terrestrial.utilities[u] & 0xff,
        (this.skills.terrestrial.utilities[u] >> 8) & 0xff
      );
      retval.push(
        this.skills.aquatic.utilities[u] & 0xff,
        (this.skills.aquatic.utilities[u] >> 8) & 0xff
      );
    }
